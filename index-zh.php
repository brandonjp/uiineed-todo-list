...
<head>
    ...
    <meta name="version" content="1.10.1">
    ...
    <link href="public/css/style.min.css?v=1.10.1" rel="stylesheet" media="screen">
    <script>
        // 自动判断语言并切换
        ...
        if (userLanguage && userLanguage !== "zh") window.location.href = (window.location.pathname === "/") ? 'index.php' : window.location.href.replace('index-zh.php', 'index.php');
    </script>
    <style>[v-cloak]{display:none}</style>
    <script>window.UIINEED_LANG = 'zh';</script>
    <script src="public/js/i18n.js?v=1.10.1"></script>
...
<body>
    ...
    <div class="todo-wrapper">
        <div id="todo-app" class="todo-app" v-cloak>
            <div class="container header ">
                <div class="todo-input">
                    ...
                    <div class="add-content-wrapper">
                        <input type="text" rows="3" class="add-content" :placeholder="t.addPlaceholder" v-model="newTodoTitle"
                            @keyup.enter="addTodo" :class='{empty:emptyChecked}' />
                        <transition name="tips">
                            <div class="tips" v-if='emptyChecked' style="color:red">{{ t.enterContent }}</div>
                        </transition>
                        <button class="btn submit-btn" type="button" @click="addTodo">{{ t.addBtn }}</button>
                    </div>
            ...
            <div class="container main">
                <div class="todo-list-box">
                    ...
                    <ul v-if="!todos.length && showEmptyTips" class="empty-tips">
                        <li v-for="(tip, i) in t.tipsList" :key="i">{{ tip }}</li>
                    </ul>

                    <div v-if="showNoMatch" class="search-no-match">
                        <span class="search-no-match-text">{{ tf('searchNoMatch', { q: searchQuery.trim() }) }}</span>
                        <button type="button" class="search-no-match-clear" @click="clearSearch">{{ t.searchClear }}</button>
                    </div>
                    ...
                    appear>
                        ...
                        <li v-for='(todo, index) in displayTodos' :key='todo.id' class='todo-item'
                            :class="{ 'touch-dragging': touchDragging && dragIndex === index, 'drag-source': isDragging && dragIndex === index }"
                            :data-index="index"
                            @dragenter="dragenter($event, index)" @dragover="dragover($event, index)"
                            @dragstart="dragstart($event, index)"
                            @dragend="dragend"
                            @touchstart="touchStartItem(index, $event)"
                            @touchmove="touchMoveItem($event)"
                            @touchend="touchEndItem"
                            @touchcancel="touchEndItem"
                            ...
                            <!-- 删除 -->
                            <div class="todo-btn btn-delete" v-if="!todo.removed && !(editedTodo !== null && editedTodo.id === todo.id)" @click="removeTodo(todo)">
                            ...
                            <div class="edit-todo-wrapper" v-if="editedTodo !== null && editedTodo.id === todo.id">
                                <input type="text" class="edit-todo" :placeholder="t.editPlaceholder"
                    ...
                    <div class="bar-message bar-bottom">
                        <div class="bar-message-text">
                            <span v-if="leftTodosCount">{{ tf('itemsRemaining', { n: leftTodosCount }) }}</span>
                ...
                <div class="footer side-bar" >
                    ...
                    <div class="todo-footer-box">
                        <!-- In-view search: narrows the currently displayed list -->
                        <div class="todo-search" v-if="todos.length">
                            <input type="text" size="1" class="todo-search-input" ref="searchInput"
                                v-model="searchQuery" :placeholder="t.searchPlaceholder"
                                :aria-label="t.searchPlaceholder"/>
                            <button type="button" class="todo-search-clear" v-if="searchActive"
                                @click="clearSearch" :title="t.searchClear" :aria-label="t.searchClear">×</button>
                        </div>
                        ...
                        <!-- One-shot sort -->
                        <div class="sort-bar" v-if="todos.length > 1">
                        ...
                        <div class="context-bar" v-if="contextActions.length">
                            <button type="button" v-for="a in contextActions" :key="a.id"
                                class="context-btn"
                                :class="{danger: a.danger, confirming: confirmId===a.id, ready: confirmId===a.id && confirmCount<=0}"
                                :disabled="confirmId===a.id && confirmCount>0"
                                @click="onContextClick(a)">{{ contextLabel(a) }}</button>
                        </div>
                        ...
                        <!-- Sync status (optional cross-device sync) -->
                        <div class="sync-status" v-if="showSyncStatus" :class="'sync-' + syncStatus"
                            @click="syncNow" role="button" tabindex="0" :title="t.syncNow">
            ...
            <!-- 批量添加弹窗 -->
            <div class="ui-modal-overlay" v-if="showBulk" @click.self="cancelBulk">
                <div class="ui-modal">
                    <div class="ui-modal-title">{{ t.bulkTitle }}</div>
                    <div class="ui-modal-hint">{{ t.bulkHint }}</div>
                    <textarea class="ui-modal-textarea" v-model="bulkText" :placeholder="t.bulkPlaceholder" rows="8"></textarea>
                    <div class="ui-modal-buttons">
                        <button class="custom-alert-btn cancel" @click="cancelBulk">{{ t.bulkCancel }}</button>
                        <button class="custom-alert-btn confirm" @click="confirmBulk">{{ t.bulkConfirm }}</button>
                    </div>
                </div>
            </div>
            <!-- 粘贴导入弹窗 -->
            <div class="ui-modal-overlay" v-if="showPaste" @click.self="cancelPaste">
                <div class="ui-modal">
                    <div class="ui-modal-title">{{ t.pasteTitle }}</div>
                    <div class="ui-modal-hint">{{ t.pasteHint }}</div>
                    <textarea class="ui-modal-textarea" v-model="pasteText" rows="8"></textarea>
                    <div class="ui-modal-buttons">
                        <button class="custom-alert-btn cancel" @click="cancelPaste">{{ t.pasteCancel }}</button>
                        <button class="custom-alert-btn confirm" @click="confirmPaste">{{ t.pasteConfirm }}</button>
                    </div>
                </div>
            </div>
            <!-- 设置弹窗 -->
            <div class="ui-modal-overlay" v-if="showSettings" @click.self="closeSettings">
                <div class="ui-modal">
                    <div class="ui-modal-title">{{ t.settingsTitle }}</div>
                    <div class="settings-row">
                        ...
                        <div class="theme-options">
                            <button
                                ...
                                @click="setTheme(opt.value)">{{ opt.label }}</button>
                        </div>
                    </div>
                    <div class="ui-modal-buttons">
                        <button class="custom-alert-btn confirm" @click="closeSettings">{{ t.settingsClose }}</button>
                    </div>
                </div>
            </div>

            <!-- Sort menu -->
            <div class="ui-modal-overlay more-overlay" v-if="showSort" @click.self="closeSort">
                <div class="more-panel">
                    <div class="more-head">
                        <span class="more-title">{{ t.sortTitle }}</span>
                        <button type="button" class="more-close" @click="closeSort" aria-label="Close">×</button>
                    </div>
            ...
            <div class="nav" draggable="false">
                <div class="github">
                    <a href="https://github.com/ricocc/uiineed-todo-list" target="_blank" rel="noopener noreferrer" class="social-link" draggable="false">
                ...
                <div class="about">
                    <div class="info">
                        ...
                        <div class="popup animated popIn" v-if="!popShow">
                            <a href="https://ricoui.com/?ref=todo" class="author" target="_blank" rel="noopener noreferrer" style="text-decoration: none;">
                            ...
                            <div class="social">
                                ...
                                <a href="https://www.xiaohongshu.com/user/profile/5f2b6903000000000101f51f" target="_blank" rel="noopener noreferrer" class="social-link">
                                    <img src="public/img/social/xiaohongshu.svg" class="ic-social" alt="">
                                </a>
                                <a href="https://x.com/ricouii" target="_blank" rel="noopener noreferrer" class="social-link">
                                    <img src="public/img/social/x.svg" class="ic-social" alt="">
                                </a>
                                <!-- <a href="https://space.bilibili.com/11442751?ref=opensource-todo" target="_blank" rel="noopener noreferrer" class="social-link">
                                    <img src="public/img/social/bilibili.svg" class="ic-social" alt="">
                                </a> -->
                                <a href="https://www.zcool.com.cn/u/13170647" target="_blank" rel="noopener noreferrer" class="social-link">
                            ...
                            <a href="https://github.com/ricocc/uiineed-todo-list" target="_blank" rel="noopener noreferrer" class="inspiration">⭐<small>200+</small> Todo开源地址 
                            </a>
                            <a href="https://ricoui.com/?ref=todo" target="_blank" rel="noopener noreferrer" class="inspiration">⭐<small>180+</small> 个人网站↗
                            </a>
                            <div class="zanshan">
                                <p>有问题可以添加微信☕</p>
                ...
                <div class="language switch-language">
                    <a href="index.php" class="en" draggable="false" @click="saveLanguage('en')">En</a>
                    <span>/</span>
    ...
    <script src="public/js/app.js?v=1.10.1"></script>
